 ...
 */
case class Bar(time: Long, high: Double, low: Double, open: Double, close: Double,  volume: Int, count: Int, wap: Double, hasGaps: Boolean) extends Ordered[Bar] {
  def compare(that: Bar): Int = time.compare(that.time)
}
